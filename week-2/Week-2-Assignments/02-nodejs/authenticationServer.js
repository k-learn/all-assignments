...
app.post("/signup", (req, res) => {
    ...
    USERS.push({ email, password, firstName, lastName, id });
    console.log(USERS);
...
app.post("/login", (req, res) => {
    ...
    for (let i = 0; i < USERS.length; ++i) {
        if (USERS[i].email === email && USERS[i].password === password) {
            return res.status(200).json({ email: USERS[i].email, firstName: USERS[i].firstName, lastName: USERS[i].lastName, id: USERS[i].id });
        }
    }
    ...
    for (let i = 0; i < USERS.length; ++i) {
        if (USERS[i].email === email && USERS[i].password === password) {
            ...
            for (let j = 0; j < USERS.length; ++j) {
                users.push({ email: USERS[i].email, firstName: USERS[i].firstName, lastName: USERS[i].lastName });
            }
            console.log({ users });
